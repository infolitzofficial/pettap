...
class CometChatUI extends React.Component {
	...
	componentDidMount(){
		let userConversationId = this.props.userConversationID;
		let ownerConversationId = `comechatuser_${this.props.petDetails.added_by.u_id}`;
		let ownerName = this.props.petDetails.added_by.u_first_name;
		let user = {
			// blockedByMe : false,
			// deactivatedAt : 0,
			// hasBlockedMe : false,
			// role : "default",
			// status : "offline",
			avatar : this.props.petDetails.added_by.u_image,
			conversationId : `${ownerConversationId}_user_${userConversationId}`,
			name : ownerName,
			uid : ownerConversationId,
		}

		this.contextProviderRef.setTypeAndItem(CometChat.ACTION_TYPE.TYPE_USER, user);
	}
